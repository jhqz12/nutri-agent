...
const emptyBodyLog: Omit<BodyLog, 'id'> = { date: new Date().toISOString().slice(0,10), weightKg: 0, waistCm: 0, sleepHours: 7, steps: 5000, cyclingMinutes: 40, energy: 5, backDiscomfort: 5, numbnessEvents: 0, trainingVolume: 0, dietAdherence: 80, morningState: 3, sleepQuality: 3, mood: '' }
...
export function ProgressView() {
  ...
  // 数值输入框的「原始文本」缓存：draft 里存的是数字，但直接把数字回填到输入框会导致
  // 1) 默认值 0 删不掉（显示成 0，继续输入变成 05、072）
  // 2) 小数点被吞掉（想输 72.5，输到 "72." 时会被 Number() 抹成 72，最后变成 725）
  // 因此输入过程中以 text 为准显示，draft 只负责保存解析后的数字。
  const [text, setText] = useState<Record<string, string | undefined>>({})
  ...
  return <div className="view-stack">
    <section className="progress-toolbar"><div className="segmented-control"><button className={range === '7' ? 'is-selected' : ''} onClick={() => setRange('7')}>7天</button><button className={range === '30' ? 'is-selected' : ''} onClick={() => setRange('30')}>30天</button><button className={range === 'all' ? 'is-selected' : ''} onClick={() => setRange('all')}>全部</button></div><button className="button primary" onClick={() => { setText({}); setShowForm(true) }}><Plus size={16} />添加今日记录</button></section>
    ...
    {showForm && <div className="dialog-backdrop"><section className="dialog wide" role="dialog" aria-modal="true"><div className="dialog-header"><h2>添加身体记录</h2><button className="icon-button" onClick={() => setShowForm(false)} aria-label="关闭"><X size={18} /></button></div><div className="form-grid three-columns">{([['date','日期','date'],['weightKg','体重kg','number'],['waistCm','腰围cm','number'],['sleepHours','睡眠小时','number'],['morningState','早上状态1～5','number'],['sleepQuality','入睡质量1～5','number'],['steps','步数','number'],['cyclingMinutes','骑行分钟','number'],['energy','精力0～10','number'],['backDiscomfort','腰背不适0～10','number'],['numbnessEvents','麻木事件次数','number'],['trainingVolume','训练总量kg','number'],['dietAdherence','饮食完成率%','number']] as const).map(([key,label,type]) => { const isNum = type === 'number'; const isDecimal = key === 'weightKg' || key === 'waistCm' || key === 'sleepHours'; const shown = isNum ? (text[key] ?? (Number(draft[key]) === 0 ? '' : String(draft[key]))) : String(draft[key]); return <label key={key}>{label}<input type={isNum ? 'text' : type} inputMode={isNum ? (isDecimal ? 'decimal' : 'numeric') : undefined} value={shown} onChange={(e) => { const raw = e.target.value; if (isNum) setText({ ...text, [key]: raw }); setDraft({ ...draft, [key]: isNum ? (raw === '' || Number.isNaN(Number(raw)) ? 0 : Number(raw)) : raw }) }} /></label> })}<label className="full-field">状态备注<textarea value={draft.mood ?? ''} onChange={(e) => setDraft({ ...draft, mood: e.target.value })} placeholder="如：早上精神不错、入睡偏晚…" /></label></div><div className="dialog-actions"><button className="button" onClick={() => setShowForm(false)}><X size={16} />取消</button><button className="button primary" onClick={addLog}><Save size={16} />保存</button></div></section></div>}
  </div>
}
